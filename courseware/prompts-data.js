...
Keep narration natural and teaching-oriented, as if presenting to a class.`
  ...
  // ---------- TRACK E — Clinical Explorer ----------
  {
    track: "explorer",
    category: "Clinical Explorer",
    title: "Google Image Search — Clinical Photos",
    blurb: "A ready-to-use Google Images search string for the visual signs of this condition.",
    text: `I want to visually study {{DISEASE}} (topic: {{TOPIC}}, {{SUBJECT}} / {{CHAPTER}}).

Generate 5 optimized Google Images search queries I can paste directly into Google to find high-quality clinical photographs, gross/histopathology images, or diagrams of this condition — covering:
1. Classic clinical presentation / external physical signs
2. Histopathology or microscopic appearance (if relevant to this subject)
3. Radiology/imaging findings (X-ray, CT, MRI, USG — whichever applies)
4. A "compare vs [closest differential]" image search
5. A labeled diagram/illustration search for exam revision

For each query, add one line explaining exactly what I should expect to see in the results and what to look for.`
  },
  {
    track: "explorer",
    category: "Clinical Explorer",
    title: "Google Search — Clinical Guidelines",
    blurb: "A ready-to-use search string for current treatment/management guidelines.",
    text: `I want the current standard-of-care guidelines for {{DISEASE}} (topic: {{TOPIC}}, {{SUBJECT}} / {{CHAPTER}}).

Generate 5 optimized Google search queries to find authoritative, up-to-date clinical guidelines — covering:
1. The most relevant international guideline body for this condition (e.g. WHO, NICE, AHA/ACC, IDSA — pick whichever applies)
2. Indian national guidelines / national health programme protocol, if one exists for this condition
3. A recent guideline update or revision (last 2-3 years)
4. A concise summary/algorithm version suitable for quick revision
5. A guideline-vs-textbook comparison search, in case standard teaching has changed

For each query, note which organization or source it's most likely to surface, and one caution about checking the publication date before relying on it.`
  },
  {
    track: "explorer",
    category: "Clinical Explorer",
    title: "PubMed Case Report Search",
    blurb: "A ready-to-use PubMed search string for published case reports on this condition.",
    text: `I want to read real published case reports on {{DISEASE}} (topic: {{TOPIC}}, {{SUBJECT}} / {{CHAPTER}}) to see how it presents in practice.

Generate 5 optimized PubMed search queries (using proper PubMed syntax with field tags and boolean operators, e.g. "[tiab]", "AND", "case reports"[Publication Type]) covering:
1. Classic/textbook presentation case reports
2. Atypical or diagnostically challenging presentations
3. Rare complications or unusual outcomes
4. Presentation in a specific population (e.g. pediatric, pregnancy, elderly — pick whichever is most clinically relevant)
5. A recent case report (last 5 years) to see current management approach

For each query, explain briefly what kind of case it's designed to surface and why that variant is worth reading.`
  },
  {
    track: "explorer",
    category: "Clinical Explorer",
    title: "YouTube Search — Teaching Videos",
    blurb: "A ready-to-use YouTube search string for lectures, OSCE demos, and procedure videos.",
    text: `I want to find good YouTube videos to learn {{DISEASE}} (topic: {{TOPIC}}, {{SUBJECT}} / {{CHAPTER}}).

Generate 5 optimized YouTube search queries covering:
1. A concise animated/whiteboard explainer of the mechanism or pathophysiology
2. A clinical examination or OSCE-style demonstration video (if this is an examinable clinical sign)
3. A "high-yield" or exam-revision style lecture aimed at medical students
4. A real patient case discussion or grand rounds-style video (if available for this condition)
5. A procedure/investigation demonstration video directly relevant to diagnosing or managing this condition

For each query, note what to look for in the video description or channel to judge whether the source is credible (e.g. medical school channel, board-certified clinician, peer-reviewed content) before trusting it.`
  }
];
