...
**Objective:** Take a single clinical case and run it through eight distinct critical-thinking lenses (framing, uncertainty, bias, evidence, testing, treatment, individualization, systems) to generate case-specific questions — surfacing what's worth double-checking before it's acted on.
...
questions. Confirm you understand, then ask me to paste
the case.
...
reassuring than its sensitivity actually justifies.
```
...
literacy) could be masquerading as "non-adherence."
```
...
```
Collect every question you generated across all 8 lenses. Present them
as a single list, grouped by the lens each came from, and reorder within
and across lenses so the questions most likely to change the current
plan come first. For each, state in one line what answering it would
change about the current plan.
```
