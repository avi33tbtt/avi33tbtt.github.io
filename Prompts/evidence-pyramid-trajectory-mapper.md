...
- **Source the paper's own reference list.** When a node is built from a specific cited study, statistic, or claim within the uploaded paper (e.g., a study in a forest plot, an outlier citation, a hub definition), identify and carry forward that paper's own citation (author/year at minimum; full reference-list entry — authors, title, journal, year — when available from the paper's bibliography/reference section).
- **In the chat-facing summary:** every named study/patient/outlier/pooled-estimate claim in the Extraction, Hub/textbook-path, and Outlier/divergence summaries must carry an inline citation back to the source (paper section, table/figure number, or — for Module F/G, where units of analysis are themselves cited studies — the original study's reference-list citation, e.g., "Kurtulmus et al. [74]").
- **Produce a standalone reference list** as its own numbered section placed at the very end of the output, after the interactive HTML file, containing full citation details (as reported in the uploaded paper's own bibliography) for every study/source referenced anywhere in the analysis — not just the outliers. For Modules F and G this means the reference list is typically most or all of the included-studies table, formatted as real citations rather than bare author/year tags.
- **In the HTML file:** every node/marker that corresponds to a specific cited source (a study in a forest plot, a patient tied to a specific case citation, a paper in a pyramid-climb diagram) must show its citation in its hover/tooltip and, where a legend or sidebar list is part of the design, in a visible "Sources" or "References" panel — not just an internal ID or study number. Do not label network nodes with anonymous placeholders (e.g., "Study 7") when the paper provides an author/year or numbered-reference citation; use the real citation as the primary label or sub-label.
- **Never fabricate a citation.** If a node's source can't be tied to a specific citation in the paper (e.g., a synthesized hub spanning several studies), cite all contributing studies rather than inventing a single attribution, or label it explicitly as a derived/aggregate node with no single source.
- This requirement holds regardless of module, output length, or whether the user explicitly asks for citations — treat it as always-on, the same way the branding requirements above are always-on.
...
- `<h1>` title (e.g. "Vibe Rounds — Evidence-Pyramid Trajectory Mapper"), weight 800, Inter or Space Grotesk, tight letter-spacing.
- A small pill/badge next to the title (like Case Bench's `Rx: reasoning, not answers`) summarizing the tool's stance, e.g. `Structure over averages`.
- A right-aligned block with: a one-line tagline, and small pill links to `https://avi33tbtt.github.io/` ("Project website") and the author's LinkedIn (`https://www.linkedin.com/in/dravinashkumargupta/`, "Dr. Avinash — LinkedIn"), `target="_blank" rel="noopener"`.

### Color system & typography (match the suite, do not invent a new palette)
- Base palette: paper `#F7FAFB`/`#FFFFFF`, ink `#0B2530`/`#0F172A`, muted text `#5B7480`/`#475569`, hairline borders `#DCE7EA`/`#E2E8F0`.
- Accent (teal/cyan): `#0891B2` (primary), `#0A6E85` / `#0E7490` (darker/hover), `#E3F4F8` (soft background).
- Alert/warning (for contradiction flags, high risk-of-bias, outlier highlights): `#C2410C` on `#FCE9DD`.
- Body font: `Inter` (400–800). Monospace/technical accents (labels, badges, nav, data values, node IDs): `IBM Plex Mono` or `DM Mono`.
- Reuse this palette for the visualization itself: hub/textbook-path elements in the accent color, outlier/divergent/contradicting elements in the alert color, everything else in muted ink/gray — do not introduce an unrelated color scheme for the diagram.
- Rounded, soft UI: 8–10px card radius, pill-shaped buttons/badges/nav links, subtle card shadow (`0 1px 2px rgba(11,37,48,.04), 0 8px 24px -16px rgba(11,37,48,.08)`).
...
> **Educational use only.** Not a diagnostic or clinical decision-making tool. Implements the [Vibe Rounds](https://avi33tbtt.github.io/) approach to reading evidence through trajectory structure rather than averages alone.

### Non-negotiables
- The suitebar, brand link, and footer must be present in every generated HTML file, regardless of which module (A–G) produced it.
- Never rebrand, rename, or drop "Vibe Rounds" from the output; never point the brand/home links anywhere other than `https://avi33tbtt.github.io/` and `https://avi33tbtt.github.io/home.html`.
- Keep the visualization's own accent/alert colors consistent with the palette above so the tool feels native to the suite rather than a standalone one-off page.
